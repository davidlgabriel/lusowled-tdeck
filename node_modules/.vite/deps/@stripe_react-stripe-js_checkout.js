...
//#region node_modules/@stripe/react-stripe-js/dist/checkout.esm.mjs
...
import_prop_types.default.any, import_prop_types.default.object;
...
var validateCheckoutContext = function validateCheckoutContext(ctx, useCase) {
	if (!ctx) throw new Error("Could not find checkout context; You need to wrap the part of your app that ".concat(useCase, " in a <CheckoutElementsProvider> or <CheckoutFormProvider> provider."));
	return ctx;
};
...
var useCheckout = function useCheckout() {
	var checkoutState = validateCheckoutContext(import_react.useContext(CheckoutContext), "calls useCheckout()").checkoutState;
	return mapStateToCheckoutResult(checkoutState);
};
var useCheckoutElements = function useCheckoutElements() {
	var checkoutState = validateCheckoutContext(import_react.useContext(CheckoutContext), "calls useCheckoutElements()").checkoutState;
	if (checkoutState.type === "success" && checkoutState.sdkKind !== "elements") throw new Error("useCheckoutElements() must be used inside <CheckoutElementsProvider>. Inside <CheckoutFormProvider>, use useCheckoutForm() instead.");
	return mapStateToCheckoutResult(checkoutState);
};
var useCheckoutForm = function useCheckoutForm() {
	var checkoutState = validateCheckoutContext(import_react.useContext(CheckoutContext), "calls useCheckoutForm()").checkoutState;
	if (checkoutState.type === "success" && checkoutState.sdkKind !== "form") throw new Error("useCheckoutForm() must be used inside <CheckoutFormProvider>. Inside <CheckoutElementsProvider>, use useCheckoutElements() instead.");
	return mapStateToCheckoutResult(checkoutState);
};
...
var CheckoutElementsProvider = function CheckoutElementsProvider(_ref) {
	...
	var _React$useState4 = _slicedToArray(import_react.useState(null), 2), stripe = _React$useState4[0], setStripe = _React$useState4[1];
	var initCalledRef = import_react.useRef(false);
	import_react.useEffect(function() {
		var isMounted = true;
					...
					if (result.type === "success") {
						...
						_sdk.on("change", function(session) {
							setState(function(prevState) {
								if (prevState.type === "success" && prevState.sdkKind === "elements") return {
									type: "success",
									sdkKind: "elements",
									sdk: prevState.sdk,
		...
		if (parsed.tag === "async") parsed.stripePromise.then(function(stripe) {
			setStripe(stripe);
	...
	import_react.useEffect(function() {
		if (prevStripe !== null && prevStripe !== rawStripeProp) console.warn("Unsupported prop change on CheckoutElementsProvider: You cannot change the `stripe` prop after setting it.");
	...
	import_react.useEffect(function() {
		...
		if (currentAppearance && hasAppearanceChanged) sdk.changeAppearance(currentAppearance);
		...
		if (currentFonts && hasFontsChanged) sdk.loadFonts(currentFonts);
	}, [
		options,
		prevOptions,
		sdk
	...
	return /*#__PURE__*/ import_react.createElement(CheckoutContext.Provider, { value: contextValue }, children);
};
...
var CheckoutFormProvider = function CheckoutFormProvider(_ref) {
			...
			if (stripe && isMounted && !initCalledRef.current) {
				...
				_sdk.loadActions().then(function(result) {
					if (result.type === "success") {
						...
						_sdk.on("change", function(session) {
							setState(function(prevState) {
								if (prevState.type === "success" && prevState.sdkKind === "form") return {
									type: "success",
									sdkKind: "form",
									sdk: prevState.sdk,
									checkoutActions: prevState.checkoutActions,
	...
	var prevStripe = usePrevious(rawStripeProp);
	import_react.useEffect(function() {
		if (prevStripe !== null && prevStripe !== rawStripeProp) console.warn("Unsupported prop change on CheckoutFormProvider: You cannot change the `stripe` prop after setting it.");
	}, [prevStripe, rawStripeProp]);
...
CheckoutFormProvider.propTypes = {
	...
	options: import_prop_types.default.shape({
		clientSecret: import_prop_types.default.oneOfType([import_prop_types.default.string, import_prop_types.default.instanceOf(Promise)]).isRequired,
		appearance: import_prop_types.default.object,
		loader: import_prop_types.default.string,
		fonts: import_prop_types.default.array,
		savedPaymentMethod: import_prop_types.default.object,
		defaultValues: import_prop_types.default.object
	}).isRequired
};
...
var BillingAddressElement = function BillingAddressElement(props) {
	...
	var merged = _objectSpread2(_objectSpread2({}, options), {}, { mode: "billing" });
	return /*#__PURE__*/ import_react.createElement(AddressElementBase, _objectSpread2(_objectSpread2({}, rest), {}, { options: merged }));
};
var ShippingAddressElement = function ShippingAddressElement(props) {
	...
	var merged = _objectSpread2(_objectSpread2({}, options), {}, { mode: "shipping" });
	return /*#__PURE__*/ import_react.createElement(AddressElementBase, _objectSpread2(_objectSpread2({}, rest), {}, { options: merged }));
};
//#endregion
